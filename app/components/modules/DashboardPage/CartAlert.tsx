...
import { ICartAlertProps } from '@/app/types/dashboard'
import { formatPrice } from '@/app/utils/common'
import { $mode } from '@/app/context/mode'
import { $totalPrice } from '@/app/context/shopping-cart'
...
const CartAlert = ({ count, closeAlert }: ICartAlertProps) => {
  ...
  const showCountMessage = (count: string) => {
    if (count.endsWith('1') && !count.endsWith('11')) {
      return 'товар'
    }
  ...
  return (
    <>
      <div className={`${styles.dashboard__alert__left} ${darkModeClass}`}>
        ...
        <span>На сумму {formatPrice(totalPrice)} руб.</span>
      </div>
